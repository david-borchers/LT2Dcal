...
load("sc1.RData")
kable(sc1.out <- result(sc1, n=600))
```

```{r Sc.2, message=FALSE, warning=FALSE, cache=FALSE}
# Scenario 2
# Initial density: uniform + Movement: avoidance + Matching: imperfect
#sc2 <- simulation(n=600, b=99, 0, 0, TRUE)
load("sc2.RData")
kable(sc2.out <- result(sc2, n=600))
```
...
load("sc3.RData")
kable(sc3.out <- result(sc3, n=600))
```
...
load("sc4.RData")
kable(sc4.out <- result(sc4, n=600))
```
...
kable(sc5.out <- result(sc5, n=1200))
```
...
kable(sc6.out <- result(sc6, n=1200))
```
...
kable(sc7.out <- result(sc7, n=1200))
```
...
load("sc8.RData")
kable(sc8.out <- result(sc8, n=1200))
```
...
load("sc9.RData")
kable(sc9.out <- result(sc9, n=400))
```
