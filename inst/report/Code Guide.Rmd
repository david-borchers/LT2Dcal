...
-   [`sim.data(n,density,move)`](#sim.data) generates detections from a population of n animals with specified density and movement. Change this function if you want a different detection function, initial density or movement (you will need to update fit.2d if you change density)
...
-   `fit.2d(df, density)` fits a 2D model to a sim.data data frame using same density function as used to generate data. Returns a point estimate and confidence interval for abundance. If you change the density settings in sim.data make sure to update the density settings in fit.2d.
