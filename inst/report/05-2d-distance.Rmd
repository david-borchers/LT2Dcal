...
hist(DistData$forward, main = "Forward Distance", xlab = "Distance")
```

From looking at the histograms of observed data we set the perpendicular truncation distance at `r w` and the forward truncation distance at `r ystart`. This excluded 2 outlier observations.\
...
total.N <- round(combined.ip0.hnorm$ests[nrow(combined.ip0.hnorm$ests),ncol(combined.ip0.hnorm$ests)])
total.ibex <- round(ibex.ip0.hnorm$ests[nrow(ibex.ip0.hnorm$ests),ncol(ibex.ip0.hnorm$ests)])
total.argali <- round(argali.ip0$ests[nrow(argali.ip0$ests),ncol(argali.ip0$ests)])
```
