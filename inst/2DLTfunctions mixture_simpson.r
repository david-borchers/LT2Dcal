...
#  return(hnF(x,logphi)/integrate(hnF,0,w,logphi)$value)
  return(hnF(x,logphi)/simpson(hnF,0,w,logphi=logphi))
}
...
#  return(chnF(x,logphi)/integrate(chnF,0,w,logphi)$value)
  return(chnF(x,logphi)/simpson(chnF,0,w,logphi=logphi))
}
...
#  denom = w - logphi[2]*int$value
  int=simpson(pi.chnorm.i.exp.term, lower=0,upper = w, logphi=logphi)
  denom = w - logphi[2]*int
